...
if (isset($_POST['insertar'])) {
    if (
        ...
        if (
            strlen($nombre) > 0 && strlen($apellido) > 0 && strlen($correo) > 0 && strlen($telefono) > 0
            ...
            foreach ($elementos as $row) {
                if ($row->getNombreTBInstructor() == $_POST['nombre'] && $row->getActivoTBInstructor() == 1 && $row->getApellidoTBInstructor() == $_POST['apellido'] &&  $row->getTelefonoTBInstructor() == $tempTelefono) {
                    $flag = 1;
                }
            }
...
if (isset($_POST['eliminar'])) {
    ...
    foreach ($facturas as $row) {
        if ($row->getInstructoridTBFactura() == $_POST['idInstructor'] && $row->getActivoTBFactura() == 1) {
            $flag = 1;
        }
    }
...
if (isset($_POST['actualizar'])) {
    ...
    if (
        isset($_POST['idInstructor']) && isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['correo'])
        && isset($_POST['telefono']) && isset($_POST['numcuenta']) && isset($_POST['tipoinstructor'])
    ) {
